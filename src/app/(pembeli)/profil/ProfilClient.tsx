        ...
        <div className="max-w-xl mx-auto relative">
          ...
          <div className="flex flex-col items-center">
            <div style={{ position: 'relative', width: 96, height: 96, marginBottom: 12 }}>
              <div style={{ width: 96, height: 96, borderRadius: 20, overflow: 'hidden', background: '#71BC68', color: '#0A4C3E', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, fontWeight: 700 }}>
                {profile.avatar_url ? (
              ...
              <button onClick={() => fileInputRef.current?.click()} disabled={uploadingAvatar}
                style={{ position: 'absolute', bottom: -2, right: -2, width: 28, height: 28, borderRadius: '50%', background: uploadingAvatar ? '#ccc' : '#71BC68', border: '2px solid #0d6b55', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            ...
            <p className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.55)' }}>Bergabung sejak {joinDate}</p>
            {uploadingAvatar && <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.7)' }}>Mengupload foto...</p>}
          </div>
        </div>
      </div>

      {/* Content area overlap header */}
        ...
        <div className="max-w-xl mx-auto px-4 pt-5 pb-24 md:pb-10">
          ...
          <div className="grid grid-cols-3 gap-3 mb-5">
            {[
              ...
              <div key={item.label} className="bg-white rounded-xl text-center"
                style={{ border: '1px solid rgba(113,188,104,0.15)', boxShadow: '0 2px 8px rgba(10,76,62,0.08)', padding: '14px 8px 12px' }}>
                <div className="w-7 h-7 rounded-lg flex items-center justify-center mx-auto mb-2" style={{ background: item.color }}>
                  <item.icon size={14} color={item.iconColor} />
                </div>
                <p className="font-bold text-sm" style={{ color: '#0A4C3E', fontFamily: 'Sora, sans-serif' }}>{item.value}</p>
