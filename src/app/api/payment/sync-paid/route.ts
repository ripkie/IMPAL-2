...
const MIDTRANS_STATUS_BASE_URL = process.env.MIDTRANS_IS_PRODUCTION === 'true'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!order.midtrans_order_id) {
      return NextResponse.json({ error: 'Midtrans order id belum tersedia' }, { status: 400 })
    }

    const midtransRes = await fetch(`${MIDTRANS_STATUS_BASE_URL}/${encodeURIComponent(order.midtrans_order_id)}/status`, {
      method: 'GET',
      headers: {
    ...
    if (!midtransRes.ok) {
      return NextResponse.json({ error: midtransData?.status_message ?? 'Gagal cek status pembayaran' }, { status: 400 })
    }

    if (!isPaidStatus(midtransData.transaction_status, midtransData.fraud_status)) {
